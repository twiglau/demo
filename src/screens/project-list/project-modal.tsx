...
export const ProjectModal = () => {
    ...
    const { mutateAsync, error, isPending:mutatePending } = useMutateProject()

    const title = editingProject ? '编辑项目' : '创建项目'
    const [ form ] = Form.useForm()

    useEffect(() => {
        form.setFields(editingProject)
    }, [editingProject, form])
        ...
        >
            {
                isLoading ? <Spin size={ 'large' } /> : <>
                   ...
                   <Form form={form} layout={'vertical'} style={{width:'40rem'}} onFinish={onFinish}>
                      ...
                      <Form.Item name='organization' label={'部门名'} rules={[{required:true,message:'请输入部门名'}]}>
